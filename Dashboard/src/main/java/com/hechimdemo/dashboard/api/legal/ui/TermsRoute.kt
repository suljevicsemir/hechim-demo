...
class RouteTerms: HechimRoute("terms")
...
fun TermsRoute(
    ...
    HechimScreen (
        config = HechimScreenConfig(
            title = stringResource(id = R.string.terms_page_title),
            errorReset = {
                legalViewModel.getTerms()
            }
        ),
        resource = legalViewModel.terms
    ){
        Column (
            modifier = Modifier
                .padding(it)
            ...
            HechimHtmlText(
                html = legalViewModel.termsContent,
